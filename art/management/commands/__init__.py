...
COLUMNS_MAP = {
    ...
    "Q periodos deudores": "q_periodos_deudores",
    "Estado contrato": "estado_contrato",
    "Email del trato": "email_trato",
...
def parse_ars(value) -> Decimal | None:
    ...
    if s == "":
        return None
    # Quitar cualquier símbolo que no sea dígito, punto o coma, y signo
    s = s.replace("$", "").replace("ARS", "").replace("U$S", "")
    s = re.sub(r"[^\d,.\-]", "", s)
    # Si viene con coma decimal latina
...
def parse_decimal(value, quant=DEC2) -> Decimal | None:
    if value is None or (isinstance(value, float) and pd.isna(value)):
        return None
...
def parse_bool_generic(value) -> bool:
    ...
    if s in FALSE_WORDS:
        return False
    # Heurística: cualquier string no vacío que no sea falso explícito -> True
    return bool(s)
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--hoja",
            default="Promecor",
            help="Nombre de la hoja a usar (por defecto 'Promecor').",
        )
        parser.add_argument(
            "--periodo",
            default=None,
    ...
    def handle(self, *args, **options):
        ...
        if faltantes:
            raise CommandError(
                f"Faltan columnas obligatorias en '{hoja}': {faltantes}. "
                f"Columnas disponibles: {cols}"
            )

        # Normalizamos strings para evitar NaNs
        for c in cols:
        ...
        with transaction.atomic():
            for i, row in df.iterrows():
                try:
                    ...
                    errores += 1
                    # Podríamos loguear la fila con más detalle si te sirve
                    raise CommandError(f"Error en fila {i+2} (contando encabezado): {e}")

        self.stdout.write(self.style.SUCCESS(
            f"Importación finalizada. Creados: {creados} | Actualizados: {actualizados} | Errores: {errores}"
        ))
