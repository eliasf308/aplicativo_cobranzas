...
def get_start_cursor(conn):
    with conn, conn.cursor() as cur:
        ...
        if row:
            return row[0]
        cur.execute("SELECT MAX(modified_time) FROM public.crm_contacts;")
        row = cur.fetchone()
        if row and row[0]:
            return row[0]
...
def fetch_page(token, ims_dt, page_token=None):
    ...
    r = requests.get(f"{API_BASE}/crm/v5/{MODULE}", headers=headers, params=params, timeout=60)
...
def sync_incremental():
    ...
    print(f"Iniciando incremental Contacts desde: {start.isoformat()}")
    ...
    try:
        while True:
            ...
            if not rows and not more and not page_token:
                print("Contacts: no hay cambios nuevos.")
                break
        ...
        if max_seen > start:
            ...
            print(f"Contacts cursor actualizado a: {max_seen.isoformat()}")
